...
window.MD_history.prototype.change = function (callback, identifier = null) {
  ...
  this.groupsCallback[identifier] = callback
...
window.MD_history.prototype.on = function (event, type, callback, identifier = null) {
  ...
  if (type === "replaceState") {
    this.replaceCallback[identifier] = callback
  }

  if (type === "pushState") {
    this.pushCallback[identifier] = callback
  }

  if (type === "back") {
    this.backCallback[identifier] = callback
  }

  if (type === "forward") {
    this.forwardCallback[identifier] = callback
  }

  if (type === "go") {
    this.goCallback[identifier] = callback
  }
